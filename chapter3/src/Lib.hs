...
msgGetSet = set msg (view msg structure) structure == structure
  where
    structure = ReallyBadError "hello"
...
buildGetSet =
  applyContext (set build (view build struct) struct)
    == applyContext struct
  where
    struct = Builder ["foo", "bar", "baz"] concat

buildSetSet =
  applyContext (set build diff (set build val struct))
...
limePrice = lens getter setter
  where
    getter = _limePrice
    setter pps newPrice =
      let price = _limePrice pps
          clamp' = clamp (price - 0.5) (price + 0.5)
       in pps {_limePrice = clamp' newPrice}
...
lemonPrice = lens getter setter
  where
    getter = _lemonPrice
    setter pps newPrice =
      let price = _lemonPrice pps
          clamp' = clamp (price - 0.5) (price + 0.5)
       in pps {_lemonPrice = clamp' newPrice}
